...
mod tests {
    ...
    fn test_sentence_match_serialization() {
        let match_item = SentenceMatch::new(
            "doc1.pdf".to_string(),
            5,
        ...
        assert!(json.contains("source_doc"));
        assert!(json.contains("doc1.pdf"));
        assert!(json.contains("0.85"));
    }

    #[test]
    fn test_global_similarity_serialization() {
        let global = GlobalSimilarity::new(
            "doc1.pdf".to_string(),
            "doc2.pdf".to_string(),
            0.72,
        );
        ...
        assert!(json.contains("docA"));
        assert!(json.contains("docB"));
        assert!(json.contains("score"));
    }

    #[test]
    fn test_full_response_serialization() {
        let metadata = AnalysisMetadata::new(3, 150, 200, 0.70);
        let matches = vec![
            SentenceMatch::new(
                "a.pdf".to_string(),
                1,
                "Sample sentence from document A.".to_string(),
                "b.pdf".to_string(),
                2,
                "Similar sentence from document B.".to_string(),
                0.91,
            ),
        ];
        let global = vec![
            GlobalSimilarity::new("a.pdf".to_string(), "b.pdf".to_string(), 0.78),
        ];
        
        let response = SentenceAnalysisResponse::new(metadata, matches, global);
        let json = serde_json::to_string(&response).unwrap();
        
        assert!(json.contains("metadata"));
        assert!(json.contains("matches"));
        assert!(json.contains("global_similarity"));
    }
}
